...
# Get
# ----------------------------------------------------------------------------
PHONY += $(EGLIBC_LINUXTHREADS_SOURCE_B)
...
$(EGLIBC_LINUXTHREADS_SOURCE_B):
	...
	if [ -d $(EGLIBC_LINUXTHREADS_SOURCE_B) ]; then \
	   ...
	   rev2=`cat $(EGLIBC_LINUXTHREADS_SOURCE)`; \
	   if test "$$rev1" != "$$rev2"; then \
	      echo $$rev1 > $(EGLIBC_LINUXTHREADS_SOURCE); \
	      echo Touched $(EGLIBC_LINUXTHREADS_SOURCE) because of changes; \
	   fi; \
	else \
